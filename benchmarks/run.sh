#!/usr/bin/env bash
#
# Copyright (c) the go-ruby-pathname authors
# SPDX-License-Identifier: BSD-3-Clause
#
# Library-level cross-runtime benchmark runner for go-ruby-pathname.
#
# Runs the SAME workload through (a) the pure-Go go-ruby-pathname/pathname
# library (benchmarks/go) and (b) each available reference Ruby runtime
# (benchmarks/ruby/pathname.rb), then prints one Markdown table per
# sub-benchmark: ns/op and the ratio vs MRI.
#
# Before timing, the Go driver's output is checked BYTE-IDENTICAL to MRI's
# (the resulting path strings for every op); a mismatch aborts. Every op is a
# PURE-PATH (lexical) manipulation — no filesystem access — so the workload is
...
echo "== go-ruby-pathname library-level benchmark ==" >&2
echo "  building go driver ..." >&2
( cd go && GOWORK=off go build -o bench . ) || { echo "go build failed" >&2; exit 1; }
...
echo "  go ..." >&2
( cd go && GOWORK=off ./bench ) | awk '$1=="RESULT"{printf "go\t%s\t%s\n", $2, $3}' >> "$TMP"
run "mri"         "$RUBY"                "$RB"
